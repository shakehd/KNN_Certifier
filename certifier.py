...
from src.abstract.certify import Result, certify_point
...
def get_args_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--all-labels", help="Compute all labels.", action="store_true")
  parser.add_argument(
    "--partition-size",
    metavar="PSIZE",
    type=int,
  ...
  parser.add_argument("--no-parallel", action="store_true", help="Classify point sequentially.")
  parser.add_argument(
    "--log ",
    dest="log_level",
  ...
  parser.add_argument(
    "--no-early-stopping",
    action="store_true",
    help="Disable early stopping optimization.",
  )
  parser.add_argument(
    "--no-majority-prunning",
    action="store_true",
    help="Disable majority prunning optimization.",
  )
  parser.add_argument(
    "--no-early-majority-detection",
    action="store_true",
    help="Disable early majority detection optimization.",
  )
  parser.add_argument(
    "--no-path-length-bounds",
    action="store_true",
    help="Disable path length bounds optimization.",
  )
  parser.add_argument(
    "--retrieve-all-labels",
    action="store_true",
    help="Consider all labels during classification.",
  )
  parser.add_argument(
    "--consider-all-permutations",
    action="store_true",
    help="Consider all samples permutations during classification.",
  )
...
def parallel_main(
  ...
  training_set, test_set, dataset_props = loader.load_datasets(params["base_dirs"]["dataset"])
  abstract_classifier.initialize(training_set, partition_size, random_state)
  ...
  with tqdm(
    ...
    with ProcessPool(
      ...
      initargs=[
        ProcessLogger.get_logger().queue,
        ProcessLogger.get_logger().level,
      ],
      max_tasks=50,
    ) as pool:
      ...
      for result in as_completed(results):
        try:
          ...
          robustness_results.append(classification_result.robustness)
          stability_results.append(classification_result.stability)
          stable_count += classification_result.stability_count
          robust_count += classification_result.robustness_count
          classified_points += 1
          pbar.set_postfix_str(
            "ROB={}%, STAB={}%".format(
              round(
                sum(robust_count) / (classified_points * len(k_values)) * 100,
                1,
              ),
              round(
                sum(stable_count) / (classified_points * len(k_values)) * 100,
                1,
              ),
            )
          )
          ...
          print(f"Classification of point {results[result]} took longer than {error.args[1]} seconds")
          timeout_tasks.append(results[result])
  ...
  overall_results: list[list[str]] = [
    ["Overall stability percentage"] + [f"{(stable_points / tot_points) * 100}" for stable_points in stable_count]
  ]

  overall_results.append(
    ["Overall robustness percentage"] + [f"{(robust_points / tot_points) * 100}" for robust_points in robust_count]
  )
  ...
  overall_results.append(
    [
      "runtime (clock time)",
      f"{time.strftime('%H:%M:%S', time.gmtime(elapsed_clock))}",
    ]
    + [""] * (len(k_values) - 1)
  )
  ...
  with (
    open(join(results_dir, "classification.csv"), "w", newline="", encoding="utf-8") as classification_file,
...
def sequential_main(
  ...
  for test_point, test_label in progress_bar:
    logger.info(
      "-- Classifying point %s %s with label %s --\n",
      classified_points + 1,
      test_point,
      test_label,
    )

    if classified_points != 98:
      classified_points += 1
      continue
  ...
  overall_results.append(
    [
      ...
      f"{time.strftime('%H:%M:%S', time.gmtime(elapsed_process))}",
    ]
    + [""] * (len(k_values) - 1)
  )
...
abstract_classifier: AbstractClassifier = None  # type: ignore
...
if __name__ == "__main__":
  try:
    ...
    abstract_classifier: AbstractClassifier = AbstractClassifier(OptimizationParams.from_args(args))
    ...
    if not args.no_parallel:
      process_logger = ProcessLogger.create_process_logger(join(log_dir, "logs.log"), args.log_level.upper())
      ...
      logging.basicConfig(
        filename=join(log_dir, "logs.log"),
        filemode="w",
